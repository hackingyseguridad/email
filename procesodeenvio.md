...
Proceso Detallado de Envío de un Correo Electrónico
1. Composición del Correo (en el Cliente de Correo "Pesado")

Abres tu cliente (ej: Outlook, Thunderbird).

Escribes el destinatario (email@dominio_destino.com).

Defines el asunto.

Redactas el cuerpo del mensaje (texto, HTML, adjuntos).
...
El servidor de destino acepta el mensaje, realiza sus propias comprobaciones (spam, virus) y, si todo está bien, lo coloca en el buzón del destinatario (usando normalmente el protocolo IMAP o lo almacena localmente).

El destinatario, al abrir su cliente de correo, descargará o verá el mensaje desde su servidor.
